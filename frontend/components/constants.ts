export const STATUS_OK = 200;
export const STATUS_CREATED = 201;
export const STATUS_NO_CONTENT = 204;
export const STATUS_UNAUTHORIZED = 401;
export const LOGIN_ALERT_TIMEOUT = 2000;
export const ICON_SIZE = 30;
export const BASE_URL = 'https://gears-camp.com';
export const API_BASE_URL = 'https://api.gears-camp.com/api/' //aws
// export const API_BASE_URL = 'http://127.0.0.1:8000/api/' //local
export const ICON_IMAGE_BASE_URL = 'https://gears-images.s3.ap-northeast-1.amazonaws.com/public/icon.png';
export const ITEM_IMAGE_BASE_URL = 'https://gears-images.s3.ap-northeast-1.amazonaws.com/item/';
export const LAYOUT_IMAGE_BASE_URL = 'https://gears-images.s3.ap-northeast-1.amazonaws.com/layout/image/';
export const DEFAULT_PAGE_WIDTH = '1300px';
export const IMAGE_SIZE_WIDTH = 800;
export const IMAGE_SIZE_HEIGHT = 600;
export const CONTENT_TYPES = {
    ITEM: 'item',
    LAYOUT: 'layout',
};
